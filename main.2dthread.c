              ...
              )
{
    ...
    // Use SCHEMA
    if (timeDimidx > -1 && strcmp(vname,"pot0")) {
    ...
    for (iter = tb; iter <= te; iter+=timeStepping) {
        ...
        if (timeDimidx > -1 && strcmp(vname,"pot0")) {
            ...
            if (t != NULL && Array_getType(t) != undefArray) {
                ...
                // timeFix is used for time step of the simulation and
            ...
            snprintf(timestr, 32, "%0*d", outidxWidth, timeFix);
...
#ifdef ESIMON 
            err = esimmon_addVariable(e, newvname, timeFix,
        ...
        if (x == NULL || Array_getType(x) == undefArray) {
            ...
            printf("x is not empty and its length is : %d\n",Array_length(x));
            min_x = min_x_in;
            max_x = max_x_in;
        }
...
int storeMesh(char *meshfile) {
    ...
    _tstart();
    y = reader_read_var_byname(df_x, yvar, yistart, yicount);
    _tend();
    _tdiff(&tp);
 ...
 * plot all variables 
 */
int plotAllVarsForTimeStep( char* datafile)
{
    ...
    for (i=0; i < df_v.nvars; i++) {
        ...
        if (!excode) {
            // DEBUG printf("call plotVar for var: %s and time: %d but where? %s\n", df_v.varinfo[i].name, timeFix, outspec);
...
int main (int argc, char * argv[])
{
    ...
    while (simulation_ended == 0){
        ...
        if (next_time_step_exists == 1){
            if (current_time_step == 1){
                ...
                plotAllVarsForTimeStep(datafile);
                latest_time_step_processed = timeFix;
                current_time_step++;
                continue;
            }
        }else{
   ...
   if (dp != NULL)
   {
       while (ep = readdir (dp)){
           ...
           outputdir1 = strdup(ep->d_name);
           DIR *dp2;
           struct dirent *ep2;
           if (!strcmp(outputdir1,".") || !strcmp(outputdir1,".."))
               continue;
           snprintf(outputdir, sizeof(outputdir), "%s/%s",outpath,outputdir1);
          
           if(is_dir(outputdir1)){
               dp2 = opendir(outputdir);
               if (dp2 != NULL){
                   while (ep2 = readdir(dp2)){
                       outputdir2 = strdup(ep2->d_name);
                       if (!strcmp(outputdir2,".") || !strcmp(outputdir2,".."))
                           continue;
                       snprintf(syscommand, sizeof(syscommand), "/ccs/proj/e2e/rbarreto/xgc_plotter/calculator_movie.sh -d %s/%s/%s %s", outpath, outputdir1, outputdir2, outputdir2);
                       system(syscommand);
                   }
                   (void) closedir(dp2);
               }
           }else{
               snprintf(syscommand, sizeof(syscommand), "/ccs/proj/e2e/rbarreto/xgc_plotter/calculator_movie.sh -d %s/%s %s", outpath, outputdir1, outputdir1);
               system(syscommand);
           }
       }
       (void) closedir (dp);
